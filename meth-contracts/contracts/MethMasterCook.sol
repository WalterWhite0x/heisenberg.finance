...
interface IMigratorToMethSwap {
    // Perform LP token migration from legacy UniswapV2 to MethSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // MethSwap must mint EXACTLY the same amount of MethSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MethMasterCook is the master of METH. He can make METH and he is a fair guy.
//
...
contract MethMasterCook is Ownable {
    ...
    // The METH TOKEN!
    MethToken public meth;
    // Dev address.
    address public devaddr;
    // METH tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early METH makers.
    ...
    // The block number when METH mining starts.
    ...
    IMigratorToMethSwap public migrator;
    ...
    event SendMethReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "MethMasterCook::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToMethSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 methForDev;
        uint256 methForCook;
        (methForDev, methForCook) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (methForDev > 0) {
            meth.mint(devaddr, methForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                meth.lock(devaddr, methForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        meth.mint(address(this), methForCook);
        pool.accMethPerShare = pool.accMethPerShare.add(methForCook.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forCook) {
        uint256 multiplier = getMultiplier(_from, _to);
        uint256 amount = multiplier.mul(REWARD_PER_BLOCK).mul(_allocPoint).div(totalAllocPoint);
        uint256 methCanMint = meth.cap().sub(meth.totalSupply());

        if (methCanMint < amount) {
            forDev = 0;
            forCook = methCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 methForCook;
            (, methForCook) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accMethPerShare = accMethPerShare.add(methForCook.mul(1e12).div(lpSupply));
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            uint256 masterBal = meth.balanceOf(address(this));
            ...
            if(pending > 0) {
                meth.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    meth.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendMethReward(msg.sender, _pid, pending, lockAmount);
            }
    ...
    // Deposit LP tokens to MethMasterCook for SUSHI allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "MethMasterCook::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from MethMasterCook.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "MethMasterCook::withdraw: not good");
    ...
    // Safe sushi transfer function, just in case if rounding error causes pool to not have enough METHs.
